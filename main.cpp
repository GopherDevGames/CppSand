...
#define P_LEFT get_temp_pixel(static_cast<uint16_t>(std::max(x-1,1)), y)
#define P_RIGHT get_temp_pixel(static_cast<uint16_t>(std::min(x+1,WINDOW_DEFAULT_W-1)), y)
...
void run_logic(){
    ...
    if (is_drawing){
        ...
        set_pixel(pos_x,pos_y, picked_type);
        set_temp_pixel(pos_x, pos_y, picked_type);
    }
    ...
    for (uint16_t y = 0; y<WINDOW_DEFAULT_H; y++){
        ...
        case P_SAND:
            if (liquids[P_DOWN]){
                set_temp_pixel(x,y,P_DOWN);
                set_temp_pixel(x,y-1,P_SAND);
            }
            else if (y>1){
                if (liquids[P_DOWN_LEFT] && x>0 ){
                    set_temp_pixel(x,y,P_DOWN_LEFT);
                    set_temp_pixel(x-1,y-1,P_SAND);
                }
                else if (liquids[P_DOWN_RIGHT] && x < WINDOW_DEFAULT_W){
                    set_temp_pixel(x,y,P_DOWN_RIGHT);
                    set_temp_pixel(x+1,y-1,P_SAND);
                }
            }
            break;
        case P_WATER:
            if (gases[P_DOWN]){
                set_temp_pixel(x,y,P_DOWN);
                set_temp_pixel(x,y-1,P_WATER);
            }
            /*else if (y>1){
                if (gases[P_LEFT] && x>0){
                    set_temp_pixel(x,y,P_LEFT);
                    set_temp_pixel(x-1,y,P_WATER);
                }
                else if (gases[P_RIGHT] && x < WINDOW_DEFAULT_W){
                    set_temp_pixel(x,y,P_RIGHT);
                    set_temp_pixel(x+1,y,P_WATER);
                }
            }
            */
